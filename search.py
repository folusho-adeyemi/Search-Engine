...
def keyword_to_titles(metadata):
    ...
    for i in range(len(metadata)):
        for j in range(len(metadata[i][4])):
            if metadata[i][4][j] not in dictionary:
                dictionary[metadata[i][4][j]] = [metadata[i][0]]
            else:
                dictionary[metadata[i][4][j]].append(metadata[i][0])
    return dictionary
...
def title_to_info(metadata):
    dictionary = {}
    for i in range(len(metadata)):
        dictionary[metadata[i][0]] = {"author": metadata[i][1], "timestamp": metadata[i][2], "length": metadata[i][3]}
    return dictionary
...
def search(keyword, keyword_to_titles):
    new = []
    for i in keyword_to_titles.keys():
        if keyword == i:
            return keyword_to_titles[i]
    return new
...
def article_length(max_length, article_titles, title_to_info):
    ...
    for i in range(len(article_titles)):
        if title_to_info[article_titles[i]]["length"] <= max_length:
            new.append(article_titles[i])
    return new
...
def key_by_author(article_titles, title_to_info):
    ...
    for i in article_titles:
        if title_to_info[i]["author"] not in dictionary:
            dictionary[title_to_info[i]["author"]] = [i]
        elif title_to_info[i]["author"] in dictionary:
            dictionary[title_to_info[i]["author"]].append(i)
    return dictionary
...
def filter_to_author(author, article_titles, title_to_info):
    author_title = []
    for i in title_to_info:
        if title_to_info[i]["author"] == author and i in article_titles:
            author_title.append(i)
    return author_title
...
def filter_out(keyword, article_titles, keyword_to_titles):
    ...
    for i in article_titles:
        if keyword in keyword_to_titles:
            if i not in keyword_to_titles[keyword]:
                new_list.append(i)
    return new_list
...
def articles_from_year(year, article_titles, title_to_info):
    ...
    start_range= int(time.mktime(start.timetuple()))
    end_range= int(time.mktime(end.timetuple()))
    new = []
    for i in article_titles:
        if title_to_info[i]["timestamp"] in range(start_range, end_range +1):
            new.append(i)
    return new
